...
PADDING_FACTOR = 0.10
DEFAULT_BACKGROUND_COLOR_BGR = (0, 0, 0) # Black

def merge_object_and_ruler(
    extracted_object_path, # Path to _02_object.tif
    scaled_ruler_path,     # Path to _07.tif
    output_base_name,      # e.g., K.23105
    # padding_px removed as parameter
    background_color=DEFAULT_BACKGROUND_COLOR_BGR,
    output_suffix="_merged.jpg",
    output_jpeg_quality=95
):
    print(f"  Merging Extracted Object '{os.path.basename(extracted_object_path)}' and Scaled Ruler '{os.path.basename(scaled_ruler_path)}'")

    try:
        # Load extracted object image
        img_object = cv2.imread(extracted_object_path)
        if img_object is None:
            raise ValueError(f"Failed to load extracted object image: {extracted_object_path}")
        ...
        print(f"    Scaled ruler loaded: {ruler_width_px}x{ruler_height_px}, Channels: {ruler_channels}")

        # --- Calculate Dynamic Padding ---
        padding_px = int(round(ruler_height_px * PADDING_FACTOR))
        print(f"    Calculated padding: {padding_px}px ({PADDING_FACTOR*100:.0f}% of ruler height)")
        # --- End Calculate Dynamic Padding ---

        # Determine canvas dimensions using calculated padding
        canvas_width_px = max(object_width_px, ruler_width_px)
        canvas_height_px = object_height_px + padding_px + ruler_height_px # Add padding to height
        print(f"    Canvas dimensions: {canvas_width_px}x{canvas_height_px}")

        # Create background canvas
        canvas = np.full((canvas_height_px, canvas_width_px, 3), background_color, dtype=np.uint8)

        # Calculate placement coordinates
        object_start_x = (canvas_width_px - object_width_px) // 2
        object_start_y = 0
        ruler_start_x = (canvas_width_px - ruler_width_px) // 2
        ruler_start_y = object_height_px + padding_px # Use calculated padding

        # Paste extracted object
        canvas[object_start_y : object_start_y + object_height_px, object_start_x : object_start_x + object_width_px] = img_object
        ...
        if roi_height <= 0 or roi_width <= 0:
            ...
            ruler_target_roi = canvas[ruler_start_y : ruler_start_y + roi_height, ruler_start_x : ruler_start_x + roi_width]
            ...
            if ruler_channels == 4:
                ...
                alpha_mask_3channel = cv2.merge([alpha_channel, alpha_channel, alpha_channel])
                bgr_ruler = img_ruler_cropped[:, :, :3]
                blended_roi = cv2.convertScaleAbs(bgr_ruler * alpha_mask_3channel + ruler_target_roi * (1.0 - alpha_mask_3channel))
                canvas[ruler_start_y : ruler_start_y + roi_height, ruler_start_x : ruler_start_x + roi_width] = blended_roi
            elif ruler_channels == 3:
                 print("    Scaled ruler is BGR, direct pasting.")
                 canvas[ruler_start_y : ruler_start_y + roi_height, ruler_start_x : ruler_start_x + roi_width] = img_ruler_cropped
                ...
                canvas[ruler_start_y : ruler_start_y + roi_height, ruler_start_x : ruler_start_x + roi_width] = bgr_ruler
            else:
                 print(f"    Warning: Unsupported number of channels ({ruler_channels}) in scaled ruler. Skipping paste.")
        ...
        if not success:
            raise IOError(f"cv2.imwrite failed to save the merged image to {output_filepath}")
        print(f"    Successfully saved merged image: {output_filepath}")

    except Exception as e:
        print(f"  ERROR during merging process for {os.path.basename(extracted_object_path)}: {e}")
        raise e
